...
COPY ./ ./
RUN mvn -B clean package -pl webpos-orders-service -am -DskipTests=true

FROM eclipse-temurin:21-jre AS run
WORKDIR /usr/src/app
COPY --from=build /usr/src/app/webpos-orders-service/target/webpos-orders-service-1.0-SNAPSHOT.jar .
CMD java -server -jar webpos-orders-service-1.0-SNAPSHOT.jar
