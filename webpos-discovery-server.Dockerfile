...
COPY ./ ./
RUN mvn -B clean package -pl webpos-discovery-server -am -DskipTests=true

FROM eclipse-temurin:21-jre AS run
WORKDIR /usr/src/app
COPY --from=build /usr/src/app/webpos-discovery-server/target/webpos-discovery-server-1.0-SNAPSHOT.jar .
CMD java -server -jar webpos-discovery-server-1.0-SNAPSHOT.jar
