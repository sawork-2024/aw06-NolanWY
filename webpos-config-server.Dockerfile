...
COPY ./ ./
RUN mvn -B clean package -pl webpos-config-server -am -DskipTests=true

FROM eclipse-temurin:21-jre AS run
WORKDIR /usr/src/app
COPY --from=build /usr/src/app/webpos-config-server/target/webpos-config-server-1.0-SNAPSHOT.jar .
CMD java -server -jar webpos-config-server-1.0-SNAPSHOT.jar
